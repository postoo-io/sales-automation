...
`sales-setup`으로 저장한 `profile.json`이 있으면 콜 브리프의 "콜에서 확인할 포인트" 또는 "추천 다음 액션" 섹션에 **우리 강점·차별화**를 콜 포인트 후보로 노출합니다.

- `brand.strengths` → 콜에서 강조할 셀링 포인트 후보 (의뢰 요구사항과 매칭되는 것 우선)
- `brand.differentiation` → "왜 우리인가" 한 줄 (이의제기 대응용)
- `company.displayName` → 자기소개 헤더

비즈니스 프로필이 없으면 이 섹션은 생략됩니다. 조회: `skills/sales-setup/scripts/profile.py --show`.

> **표준 변수 참조**: 콜 브리프 내 담당자 정보 등은 `{{me.name}}`, `{{me.signature}}`, `{{client.companyName}}` 같은 표준 변수로 채워집니다. 자세한 변수 명세는 `skills/sales-setup/references/template_variables.md` 참조.
