...
권장: 프로젝트 루트에 `.env` 파일 생성 (이미 `.gitignore`에 포함됨). `scripts/pluuug.py`가 실행 시 자동으로 `.env`를 로드합니다 — 셸 export 없이도 동작.

```bash
# .env
PLUUUG_API_KEY=...      # X-API-Key 헤더 값
PLUUUG_SECRET_KEY=...   # 요청 본문 HMAC-SHA256 서명용
```
