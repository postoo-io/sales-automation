...
| 스킬 | 언제 트리거되나 | 보조 자원 |
|---|---|---|
| [`pluuug-setup`](skills/pluuug-setup/SKILL.md) | "pluuug 키 설정", "Pluuug API 등록", "키 영구 저장", "회사 정보 저장", "비즈니스 프로필 설정", "메일 서명 등록", "cowork에서 키 안 풀린다" | `scripts/install_credentials.py` (키 0600 영구 저장) · `scripts/business_info.py` (자사 비즈니스 프로필) · `references/business_schema.md` |
...
**권장: OS 표준 위치에 1회 영구 저장** — 매 세션 환경 변수를 다시 설정할 필요가 없습니다. `pluuug-setup` 스킬이 자동화합니다:
...
자사 정보(법인명·산업·강점·서명·기본 결제조건 등)를 같은 디렉토리의 `business.json`에 저장하면 영업 스킬들이 메일·견적·콜에서 일관된 자기소개와 기본값을 사용합니다.
...
스키마 전체: [`skills/pluuug-setup/references/business_schema.md`](skills/pluuug-setup/references/business_schema.md).

| 활용 | 어느 스킬 | 어떤 필드 |
|---|---|---|
| 메일 서명·자기소개·톤 | `email-send` | `voice.signatureLines`, `business.strengths`, `voice.tone` |
...
| 콜 강점·차별화 가이드 | `call-prep` | `business.strengths`, `business.differentiation` |
...
├── skills/
│   ├── pluuug-setup/                # 1회 키 설정 (OS 표준 위치 영구 저장)
│   │   ├── SKILL.md
